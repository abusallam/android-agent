...
class DomainSSLTests {
    static async runTests() {
        ...
        try {
            const result = await TestUtils.executeOnVPS(`curl -I -H 'Host: tac.consulting.sa' https://127.0.0.1/api/health -k -s`);
        ...
        try {
            const result = await TestUtils.executeOnVPS(`curl -I http://127.0.0.1 -H 'Host: tac.consulting.sa' -s`);
        ...
        try {
            const result = await TestUtils.executeOnVPS(`curl -I -H 'Host: tac.consulting.sa' https://127.0.0.1/ -k -s`);
            if (result.success) {
